"""
FRUNZEBOT – Phase 1 (MVP)
─────────────────────────
• прийом текстів / фото / відео / альбомів
• вибір категорії
• адмін-кнопки ✅ ✏️ ❌
• публікація схвалених дописів у @frunze_pro
• фідбеки автору
• коментар адміна + дедлайн 30 хв на правки
"""

from __future__ import annotations
import logging, os, time
from enum import Enum, auto
from datetime import datetime, timedelta

from tinydb import TinyDB, Query
from telegram import (
    Update, InlineKeyboardButton, InlineKeyboardMarkup,
    ReplyKeyboardMarkup, ReplyKeyboardRemove
)
from telegram.constants import ParseMode
from telegram.ext import (
    Application, CommandHandler, MessageHandler,
    CallbackQueryHandler, ContextTypes, filters, JobQueue
)

# ─────────────── Конфіг ───────────────
BOT_TOKEN   = os.getenv("BOT_TOKEN", "").strip()
ADMIN_ID    = int(os.getenv("ADMIN_ID", "0").strip() or 0)
CHANNEL_ID  = "@frunze_pro"             # ← куди публікувати схвалене

if not BOT_TOKEN or ADMIN_ID == 0:
    raise RuntimeError("Задай BOT_TOKEN і ADMIN_ID у Variables!")

# ─────────────── Дані ───────────────
class Status(Enum):
    PENDING   = auto()
    NEED_EDIT = auto()
    APPROVED  = auto()
    REJECTED  = auto()
    EXPIRED   = auto()

CATEGORIES = (
    "Інформаційна отрута",
    "Суспільні питання",
    "Я і моя філософія",
)

db = TinyDB("frunze_drafts.json")
DQ = Query()

def save(d): db.upsert(d, DQ.draft_id == d["draft_id"])
def get(did): r = db.search(DQ.draft_id == did); return r[0] if r else None
def now_iso(): return datetime.utcnow().isoformat(timespec="seconds")

# ─────────────── Хендлери ───────────────
async def cmd_start(upd: Update, _):
    kb = ReplyKeyboardMarkup(
        [["Запропонувати пост", "Поділитись новиною"],
         ["Анонімний інсайд",  "Підтримати автора"]],
        resize_keyboard=True
    )
    await upd.message.reply_text(
        "Тут формують, а не споживають.\n\n"
        "Надішли текст / фото / відео й обери категорію.",
        reply_markup=kb
    )

async def handle_payload(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Записуємо перше повідомлення й просимо категорію."""
    ctx.user_data["payload"] = upd.message
    cat_kb = ReplyKeyboardMarkup([[c] for c in CATEGORIES],
                                 resize_keyboard=True, one_time_keyboard=True)
    await upd.message.reply_text("Оберіть категорію:", reply_markup=cat_kb)

async def choose_category(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    cat = upd.message.text
    payload = ctx.user_data.pop("payload", None)
    if cat not in CATEGORIES or payload is None:
        return

    draft_id = f"{payload.chat.id}_{payload.id}"
    draft = dict(
        draft_id=draft_id,
        user_id=payload.from_user.id,
        status=Status.PENDING.name,
        category=cat,
        date=now_iso(),
        expire_at=(datetime.utcnow() + timedelta(minutes=30)).timestamp()  # дедлайн на правки
    )
    save(draft)

    caption = f"<b>Категорія:</b> {cat}\n<b>ID:</b> <code>{draft_id}</code>"
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("✅", callback_data=f"approve:{draft_id}"),
        InlineKeyboardButton("✏️", callback_data=f"edit:{draft_id}"),
        InlineKeyboardButton("❌", callback_data=f"reject:{draft_id}")
    ]])

    try:
        await payload.copy(chat_id=ADMIN_ID, caption=caption,
                           reply_markup=kb, parse_mode=ParseMode.HTML)
    except Exception as e:
        logging.warning("Copy failed: %s", e)
        await ctx.bot.send_message(chat_id=ADMIN_ID, text=f"{caption}\n\n[Медіа недоступне]")

    await payload.reply_text("Дякую! Чернетка передана модератору.",
                             reply_markup=ReplyKeyboardRemove())

async def mod_callback(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if upd.effective_user.id != ADMIN_ID:
        await upd.callback_query.answer("Недостатньо прав", show_alert=True); return
    action, did = upd.callback_query.data.split(":")
    draft = get(did)
    if not draft:
        await upd.callback_query.answer("Чернетку не знайдено", show_alert=True); return

    async def ping(txt): 
        try: await ctx.bot.send_message(draft["user_id"], txt)
        except Exception as e: logging.warning("Notify fail: %s", e)

    if action == "approve":
        draft["status"] = Status.APPROVED.name; save(draft)
        await ping("✅ Ваш допис схвалено й буде опубліковано.")
        # публікація у канал (@frunze_pro). Використовуємо copy, щоб зберегти медіа/текст.
        try:
            orig_msg = await ctx.bot.forward_message(
                chat_id=CHANNEL_ID,
                from_chat_id=draft["user_id"],
                message_id=int(did.split("_")[1])
            )
            # можна додати реакції, статистику тощо
        except Exception as e:
            logging.error("Publish failed: %s", e)
        await upd.callback_query.edit_message_reply_markup(None)

    elif action == "reject":
        draft["status"] = Status.REJECTED.name; save(draft)
        await ping("❌ Ваш допис не пройшов модерацію.")
        await upd.callback_query.edit_message_reply_markup(None)

    elif action == "edit":
        draft["status"] = Status.NEED_EDIT.name; save(draft)
        ctx.user_data["edit_draft"] = did
        await upd.callback_query.message.reply_text(
            "Напишіть, що потрібно підправити:", reply_markup=ReplyKeyboardRemove())
        await upd.callback_query.answer()

async def admin_comment(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    did = ctx.user_data.pop("edit_draft", None)
    if not did: return
    draft = get(did)
    if not draft: return
    await ctx.bot.send_message(
        draft["user_id"],
        f"✏️ Ваш допис потребує правок:\n\n{upd.message.text}\n\n"
        "Надішліть нову версію у відповідь (у вас 30 хв)."
    )
    await upd.message.reply_text("Коментар надіслано.")

# ─────────────── Джоб-чистильник ───────────────
async def cleanup_expired(ctx: ContextTypes.DEFAULT_TYPE):
    """Щоп’ять хвилин переводимо прострочені чернетки в EXPIRED."""
    threshold = time.time()
    for d in db:
        if d["status"] == Status.NEED_EDIT.name and d["expire_at"] < threshold:
            d["status"] = Status.EXPIRED.name
            save(d)
            try:
                await ctx.bot.send_message(
                    d["user_id"],
                    "⏰ Час на правки минув. Заявка закрита автоматично."
                )
            except Exception: pass

# ─────────────── Запуск ───────────────
def main():
    logging.basicConfig(level=logging.INFO,
        format="%(asctime)s | %(levelname)-8s | %(name)s: %(message)s")

    app = Application.builder().token(BOT_TOKEN).build()
    jq: JobQueue = app.job_queue
    jq.run_repeating(cleanup_expired, interval=300, first=300)  # кожні 5 хв

    app.add_handler(CommandHandler("start", cmd_start))
    app.add_handler(MessageHandler(filters.TEXT & filters.User(ADMIN_ID), admin_comment))
    app.add_handler(
        MessageHandler(
            filters.ALL
            & ~filters.COMMAND
            & ~filters.Regex(f"^({'|'.join(CATEGORIES)})$")
            & ~filters.User(ADMIN_ID),
            handle_payload
        )
    )
    app.add_handler(MessageHandler(filters.Regex(f"^({'|'.join(CATEGORIES)})$"), choose_category))
    app.add_handler(CallbackQueryHandler(mod_callback))

    logging.info("Bot starting…")
    app.run_polling(stop_signals=None)

if __name__ == "__main__":
    main()